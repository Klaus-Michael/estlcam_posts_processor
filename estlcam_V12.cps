 /**
  ...
  $Date: 2025-01-30 16:40:33 $
 
*/

description = "Generic EstlCAM V12 based on LinuxCNC (EMC2) mod by KMS";
vendor = "ToniBaier mod by KMS";
vendorUrl = "https://github.com/Klaus-Michael/estlcam_posts_processor";
legal = "Copyright (C)2017-06-06 by Toni Baier mod by KMS 2025-01-30";
...
allowedCircularPlanes = 1 << PLANE_XY; // only XY // allow any circular motion
...
// map M7 to M8 to reflect EstlCAM usage: M8=Coolant ON, M9=Coolant OFF, M10=Second Coolant ON, M11= second cooland off, will turn off both coolants when calling COMMAND_COOLANT_OFF
...
function onSection() {
  ...
  if (insertToolCall ||
	...
	if(properties.notifyOnSpindleRPMChange)
	{		
		writeBlock("M0 set Spindel Speed to "+ tool.spindleRPM);
	}
